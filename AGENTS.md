...
Linting/formatting/security scanning strategy implemented per [ADR-00009](./docs/adrs/00009-linting-formatting-and-security-scanning.md): ESLint (sonarjs/import-x/security plugins) + Prettier + prettier-plugin-tailwindcss are wired in and passing; `lefthook.yml` has pre-commit (lint, format-autofix, Gitleaks) and pre-push (typecheck) hooks, tested directly (not just written); `knip.json` is tuned to a clean baseline (warn-only, not a gate); `pnpm run health` runs the repo health script (Knip + `pnpm audit` + osv-scanner). `.github/workflows/ci.yml` (PR CI) and `repo-health.yml` (scheduled) exist but are **unverified** — written and YAML-validated locally, not yet exercised by a real push/PR. `renovate.json` exists and passes Renovate's own validator; actually enabling the Renovate GitHub App is an account-level action outside what an agent can do. Gitleaks is a required local install (like lefthook) — the pre-commit hook hard-fails without it, by design.
