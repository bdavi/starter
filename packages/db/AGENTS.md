...
- `client.ts` and `env.ts` both `import "server-only"` — DB access and `DATABASE_URL` should never reach a client bundle, same reasoning as `packages/auth`. `jest.config.cts` needs `testEnvironmentOptions.customExportConditions: ["node", "react-server"]` for tests to import these files at all (`server-only` throws under Jest's default resolution otherwise) — don't remove it.
